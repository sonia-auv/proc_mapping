...
namespace coder
{
  ...
  pointCloud *pointCloud::b_select(const ::coder::array<bool, 2U> &varargin_1, ::
    coder::vision::internal::codegen::Kdtree *iobj_0, pointCloud *iobj_1) const
  {
  ...
  void e_pointCloud::buildKdtree()
  {
    ...
    if (!b_Kdtree->IsIndexed) {
      ...
      if (y) {
        ...
        half = 0;
        exitg1 = false;
        while ((!exitg1) && (half < 7439)) {
          if (!wasnan[half]) {
            idx++;
            notnan[idx - 1] = static_cast<short>(half + 1);
            if (idx >= 7439) {
              exitg1 = true;
            } else {
              half++;
      ...
      idx = static_cast<int>(rt_powd_snf(2.0, cc + 1.0) - 1.0) * 3;
        ...
        if (idxTemp1[static_cast<int>(currentNode) - 1].f1.size(1) <= 50) {
          ...
          for (i = 0; i < 3; i++) {
            for (i1 = 0; i1 < loop_ub; i1++) {
              b_inputData[i1 + b_inputData.size(0) * i] = inputData[(iidx[i1] +
                7439 * i) - 1];
            }
          }

          internal::maximum(b_inputData, fv);
          b_inputData.set_size(iidx.size(0), 3);
          loop_ub = iidx.size(0);
          for (i = 0; i < 3; i++) {
            for (i1 = 0; i1 < loop_ub; i1++) {
          ...
          float fv1[3];
          internal::minimum(b_inputData, fv1);
          fv[0] -= fv1[0];
          fv[1] -= fv1[1];
          fv[2] -= fv1[2];
          internal::maximum(fv, &p, &idx);
          ...
          double temp[3];
          double b_temp_tmp;
          half = static_cast<int>(std::ceil(static_cast<double>(sx.size(0)) /
            2.0));
          p = (sx[half - 1] + sx[half]) / 2.0F;
          cutDimTemp[static_cast<int>(currentNode) - 1] = idx;
      ...
      b_this->CutDim.set_size(half, 1);
      for (i = 0; i < half; i++) {
        b_this->CutDim[i] = cutDimTemp[i];
      }

      b_this->CutVal.set_size(half, 1);
      for (i = 0; i < half; i++) {
        b_this->CutVal[i] = cutValTemp[i];
      }

      b_this->LowerBounds.set_size(half, 3);
      for (i = 0; i < 3; i++) {
        for (i1 = 0; i1 < half; i1++) {
      ...
      b_this->UpperBounds.set_size(half, 3);
      for (i = 0; i < 3; i++) {
        for (i1 = 0; i1 < half; i1++) {
      ...
      b_this->LeftChild.set_size(half, 1);
      for (i = 0; i < half; i++) {
        b_this->LeftChild[i] = leftChildTemp[i];
      }

      b_this->RightChild.set_size(half, 1);
      for (i = 0; i < half; i++) {
        b_this->RightChild[i] = rightChildTemp[i];
      }

      b_this->LeafNode.set_size(half, 1);
      for (i = 0; i < half; i++) {
  ...
  //
  // Arguments    : void
  // Return Type  : g_pointCloud
  //
  g_pointCloud::g_pointCloud()
  {
    matlabCodegenIsDeleted = true;
  }
  ...
  //
  // Arguments    : void
  // Return Type  : i_pointCloud
  //
  i_pointCloud::i_pointCloud()
  {
    matlabCodegenIsDeleted = true;
  }
  ...
  //
  // Arguments    : void
  // Return Type  : h_pointCloud
  //
  h_pointCloud::h_pointCloud()
  {
    matlabCodegenIsDeleted = true;
  }
  ...
  //
  // Arguments    : ::coder::array<double, 2U> &location
  ...
  // Return Type  : void
  //
  void d_pointCloud::findNeighborsInRadius(const double varargin_1[3], double
    varargin_2, ::coder::array<unsigned int, 1U> &indices)
  {
        ...
        if (guard1) {
          ...
          while (nodeStack.size(0) != 0) {
            ...
            if (vision::internal::codegen::Kdtree::boundsOverlapBall(varargin_1,
                 allDists, upBounds, powRadius, static_cast<double>(X.size(1))))
            {
              if (!b_this->LeafNode[static_cast<int>(currentNode) - 1]) {
                ...
                vision::internal::codegen::Kdtree::searchNodeRadius(X,
                  varargin_1, nodeIdxThis, powRadius, &pq);
              }
            }
          }
        }
  ...
  //
  // Arguments    : ::coder::array<double, 2U> &xlim
  // Return Type  : void
  //
  ...
  //
  // Arguments    : ::coder::array<double, 2U> &ylim
  // Return Type  : void
  //
  ...
  //
  // Arguments    : ::coder::array<double, 2U> &zlim
  // Return Type  : void
  //
  ...
  //
  // Arguments    : const f_pointCloud *varargin_1
  // Return Type  : f_pointCloud *
  //
  f_pointCloud *f_pointCloud::horzcat(const f_pointCloud *varargin_1)
  {
    f_pointCloud *this_;
    ...
    int loop_ub;
    this_ = this;
    dataArray.set_size(1, 0);
    i = this_->PointCloudArrayData.size(0) * this_->PointCloudArrayData.size(1)
      - 1;
    for (int n{0}; n <= i; n++) {
      r.set_size(this_->PointCloudArrayData.size(0) *
                 this_->PointCloudArrayData.size(1));
      loop_ub = this_->PointCloudArrayData.size(0) *
        this_->PointCloudArrayData.size(1);
      for (int i1{0}; i1 < loop_ub; i1++) {
        r[i1] = this_->PointCloudArrayData[i1];
      }

      loop_ub = dataArray.size(1) + 1;
      dataArray.set_size(1, dataArray.size(1) + 1);
      dataArray[loop_ub - 1] = r[n];
    }

    i = varargin_1->PointCloudArrayData.size(0) *
      varargin_1->PointCloudArrayData.size(1) - 1;
    for (int n{0}; n <= i; n++) {
      r.set_size(varargin_1->PointCloudArrayData.size(0) *
                 varargin_1->PointCloudArrayData.size(1));
      loop_ub = varargin_1->PointCloudArrayData.size(0) *
        varargin_1->PointCloudArrayData.size(1);
      for (int i1{0}; i1 < loop_ub; i1++) {
        r[i1] = varargin_1->PointCloudArrayData[i1];
      }
    ...
    this_->PointCloudArrayData.set_size(1, dataArray.size(1));
    loop_ub = dataArray.size(1);
    for (i = 0; i < loop_ub; i++) {
      this_->PointCloudArrayData[i] = dataArray[i];
    }
  ...
  //
  // Arguments    : const ::coder::array<float, 2U> &varargin_1
  ...
  // Return Type  : b_pointCloud *
  //
  b_pointCloud *b_pointCloud::init(const ::coder::array<float, 2U> &varargin_1,
    const ::coder::array<unsigned char, 2U> &varargin_3, const ::coder::array<
    float, 2U> &varargin_5, const ::coder::array<float, 2U> &varargin_7, ::coder::
    vision::internal::codegen::Kdtree *iobj_0)
  {
    ...
    this_->Intensity.set_size(varargin_7.size(0), varargin_7.size(1));
    ...
    r.Location.set_size(this_->Location.size(0), 3);
    loop_ub = this_->Location.size(0) * 3;
    for (int i{0}; i < loop_ub; i++) {
      r.Location[i] = this_->Location[i];
    }

    r.Normal.set_size(this_->Normal.size(0), this_->Normal.size(1));
    loop_ub = this_->Normal.size(0) * this_->Normal.size(1);
    for (int i{0}; i < loop_ub; i++) {
      r.Normal[i] = this_->Normal[i];
    }

    r.Color.set_size(this_->Color.size(0), this_->Color.size(1));
    loop_ub = this_->Color.size(0) * this_->Color.size(1);
    for (int i{0}; i < loop_ub; i++) {
      r.Color[i] = this_->Color[i];
    }

    r.Intensity.set_size(this_->Intensity.size(0), this_->Intensity.size(1));
    loop_ub = this_->Intensity.size(0) * this_->Intensity.size(1);
    for (int i{0}; i < loop_ub; i++) {
      r.Intensity[i] = this_->Intensity[i];
    }
  ...
  // Return Type  : g_pointCloud *
  //
  g_pointCloud *g_pointCloud::init()
  {
  ...
  //
  // Arguments    : ::coder::vision::internal::codegen::Kdtree *iobj_0
  //                b_pointCloud *iobj_1
  // Return Type  : b_pointCloud *
  //
  b_pointCloud *pointCloud::makeEmptyPtCloud(::coder::vision::internal::codegen::
    Kdtree *iobj_0, b_pointCloud *iobj_1)
  {
    b_pointCloud *obj;
    pointclouds::internal::codegen::pc::b_pointCloudArray r;
    int loop_ub;
    iobj_1->Location.set_size(0, 3);
    iobj_1->Color.set_size(0, 3);
    iobj_1->Normal.set_size(0, 3);
    iobj_1->Intensity.set_size(0, 0);
    iobj_1->RangeData.set_size(0, 0);
    r.Location.set_size(iobj_1->Location.size(0), 3);
    loop_ub = iobj_1->Location.size(0) * 3;
    for (int i{0}; i < loop_ub; i++) {
      r.Location[i] = iobj_1->Location[i];
    }

    r.Normal.set_size(iobj_1->Normal.size(0), iobj_1->Normal.size(1));
    loop_ub = iobj_1->Normal.size(0) * iobj_1->Normal.size(1);
    for (int i{0}; i < loop_ub; i++) {
      r.Normal[i] = iobj_1->Normal[i];
    }

    r.Color.set_size(iobj_1->Color.size(0), iobj_1->Color.size(1));
    loop_ub = iobj_1->Color.size(0) * iobj_1->Color.size(1);
    for (int i{0}; i < loop_ub; i++) {
      r.Color[i] = iobj_1->Color[i];
    }

    r.Intensity.set_size(iobj_1->Intensity.size(0), iobj_1->Intensity.size(1));
    loop_ub = iobj_1->Intensity.size(0) * iobj_1->Intensity.size(1);
    for (int i{0}; i < loop_ub; i++) {
      r.Intensity[i] = iobj_1->Intensity[i];
    }
    ...
    obj = iobj_1;
    iobj_1->PointCloudArrayData.set_size(0, 0);
    return obj;
  }
  ...
  // Return Type  : void
  //
  void g_pointCloud::matlabCodegenDestructor()
  {
    if (!matlabCodegenIsDeleted) {
      matlabCodegenIsDeleted = true;
    }
  ...
  // Return Type  : void
  //
  void h_pointCloud::matlabCodegenDestructor()
  {
    if (!matlabCodegenIsDeleted) {
      matlabCodegenIsDeleted = true;
    }
  ...
  // Return Type  : void
  //
  void i_pointCloud::matlabCodegenDestructor()
  {
    if (!matlabCodegenIsDeleted) {
      matlabCodegenIsDeleted = true;
    }
  ...
  //
  // Arguments    : vision::internal::codegen::Kdtree *iobj_0
  //                b_pointCloud *iobj_1
  // Return Type  : b_pointCloud *
  //
  b_pointCloud *f_pointCloud::parenReference(vision::internal::codegen::Kdtree
    *iobj_0, b_pointCloud *iobj_1) const
  {
    b_pointCloud *this1;
    array<pointclouds::internal::codegen::pc::b_pointCloudArray, 2U> dataArray;
    array<pointclouds::internal::codegen::pc::b_pointCloudArray, 1U> r;
    int loop_ub;
    this1 = pointCloud::makeEmptyPtCloud(iobj_0, iobj_1);
    dataArray.set_size(1, 1);
    r.set_size(PointCloudArrayData.size(0) * PointCloudArrayData.size(1));
    loop_ub = PointCloudArrayData.size(0) * PointCloudArrayData.size(1);
    for (int i{0}; i < loop_ub; i++) {
      r[i] = PointCloudArrayData[i];
    }
    ...
    loop_ub = dataArray[0].Location.size(0) * 3;
    for (int i{0}; i < loop_ub; i++) {
      this1->Location[i] = dataArray[0].Location[i];
    }
    ...
    loop_ub = dataArray[0].Color.size(0) * dataArray[0].Color.size(1);
    for (int i{0}; i < loop_ub; i++) {
      this1->Color[i] = dataArray[0].Color[i];
    }
    ...
    loop_ub = dataArray[0].Normal.size(0) * dataArray[0].Normal.size(1);
    for (int i{0}; i < loop_ub; i++) {
      this1->Normal[i] = dataArray[0].Normal[i];
    }

    this1->Intensity.set_size(dataArray[0].Intensity.size(0),
      this1->Intensity.size(1));
    this1->Intensity.set_size(this1->Intensity.size(0), dataArray[0].
      Intensity.size(1));
    loop_ub = dataArray[0].Intensity.size(0) * dataArray[0].Intensity.size(1);
    for (int i{0}; i < loop_ub; i++) {
      this1->Intensity[i] = dataArray[0].Intensity[i];
    }
  ...
  //
  // Arguments    : d_pointCloud *iobj_0
  //                d_pointCloud **ptCloudOut
  //                ::coder::array<double, 1U> &indicesOut
  // Return Type  : void
  //
  void pointCloud::removeInvalidPoints(d_pointCloud *iobj_0, d_pointCloud
  ...
  //
  // Arguments    : const ::coder::array<double, 1U> &indices
  ...
  //
  // Arguments    : const ::coder::array<bool, 1U> &indices
  ...
  // Return Type  : void
  //
  void b_pointCloud::subsetImpl(const ::coder::array<bool, 1U> &indices, ::coder::
    array<float, 2U> &loc, ::coder::array<unsigned char, 2U> &c, ::coder::array<
    float, 2U> &nv, ::coder::array<float, 2U> &intensity, ::coder::array<float,
    2U> &r) const
  {
    array<int, 1U> b_r;
    array<int, 1U> r1;
    ...
    if (Location.size(0) != 0) {
      ...
      for (int i{0}; i < 3; i++) {
        for (int b_i{0}; b_i < trueCount; b_i++) {
          loc[b_i + loc.size(0) * i] = Location[(b_r[b_i] + Location.size(0) * i)
            - 1];
        }
      }
    } else {
    ...
    if ((Intensity.size(0) != 0) && (Intensity.size(1) != 0)) {
      ...
      trueCount = Intensity.size(1);
      intensity.set_size(r3.size(0), trueCount);
      for (int i{0}; i < trueCount; i++) {
        end = r3.size(0);
        for (int b_i{0}; b_i < end; b_i++) {
          intensity[b_i + intensity.size(0) * i] = Intensity[(r3[b_i] +
            Intensity.size(0) * i) - 1];
        }
      }
    } else {
    ...
    if ((RangeData.size(0) != 0) && (RangeData.size(1) != 0)) {
      ...
      for (int i{0}; i < trueCount; i++) {
        ...
        for (int b_i{0}; b_i < end; b_i++) {
          r[b_i + r.size(0) * i] = RangeData[(r4[b_i] + RangeData.size(0) * i) -
            1];
        }
      }
    } else {
  ...
  //
  // Arguments    : ::coder::array<float, 2U> &normals
  // Return Type  : void
  //
  void i_pointCloud::surfaceNormalImpl(::coder::array<float, 2U> &normals)
  {
    vision::internal::codegen::c_Kdtree *b_this;
    array<double, 2U> cutDim;
    array<double, 2U> cutVal;
    ...
    int nxout;
    int yk;
    nxout = Location.size(0);
    yk = Location.size(0);
    if (yk <= 2) {
      unsigned int unnamed_idx_0;
      unnamed_idx_0 = static_cast<unsigned int>(Location.size(0));
      normals.set_size(static_cast<int>(unnamed_idx_0), 3);
      yk = static_cast<int>(unnamed_idx_0) * 3;
      for (int i{0}; i < yk; i++) {
        normals[i] = rtNaNF;
      }
    } else {
      int i;
      loc.set_size(Location.size(0), 3);
      yk = Location.size(0) * 3;
      for (i = 0; i < yk; i++) {
        loc[i] = Location[i];
      }
      ...
      any(r, wasNaNY);
      yk = static_cast<int>(std::fmin(std::fmin(6.0, static_cast<double>(nxout)),
        static_cast<double>(X.size(0))));
      if ((loc.size(0) == 0) || (yk == 0)) {
        indices.set_size(yk, loc.size(0));
        yk *= loc.size(0);
        for (i = 0; i < yk; i++) {
          indices[i] = 0U;
        }

        valid.set_size(loc.size(0));
        yk = loc.size(0);
        for (i = 0; i < yk; i++) {
          valid[i] = 0U;
        }
        ...
        if (numNN > 0) {
          ...
          for (int j{0}; j < i; j++) {
            if (!wasNaNY[j]) {
              ...
              leafNode.set_size(b_this->LeafNode.size(0), b_this->LeafNode.size
                                (1));
              ...
              startNode = 1.0;
              while (!leafNode[static_cast<int>(startNode) - 1]) {
                if (loc[j + loc.size(0) * (static_cast<int>(cutDim[static_cast<
                      int>(startNode) - 1]) - 1)] <= cutVal[static_cast<int>
                    (startNode) - 1]) {
                  startNode = leftChild[static_cast<int>(startNode) - 1];
                } else {
                  startNode = rightChild[static_cast<int>(startNode) - 1];
              ...
              vision::internal::codegen::Kdtree::getNodeFromArray(b_this->IdxAll,
                b_this->IdxDim, startNode, nodeIdxThis);
              b_loc[0] = loc[j];
              b_loc[1] = loc[j + loc.size(0)];
              b_loc[2] = loc[j + loc.size(0) * 2];
              vision::internal::codegen::Kdtree::searchNode(X, b_loc,
                nodeIdxThis, numNN, &pq);
              if (pq.D.size(0) != 0) {
                ...
                if (lowBounds.size(1) == 3) {
                  ...
                  pRadIn[1] = loc[j + loc.size(0)] - static_cast<float>
                    (lowBounds[1]);
                  pRadIn[2] = loc[j + loc.size(0) * 2] - static_cast<float>
                    (lowBounds[2]);
                } else {
                  b_binary_expand_op(pRadIn, loc, j, lowBounds);
                }

                if (upBounds.size(1) == 3) {
                  b_pRadIn[0] = loc[j] - static_cast<float>(upBounds[0]);
                  b_pRadIn[1] = loc[j + loc.size(0)] - static_cast<float>
                    (upBounds[1]);
                  b_pRadIn[2] = loc[j + loc.size(0) * 2] - static_cast<float>
                    (upBounds[2]);
                } else {
                  b_binary_expand_op(b_pRadIn, loc, j, upBounds);
                }

                b_loc[0] = pRadIn[0] * pRadIn[0];
                b_loc[1] = pRadIn[1] * pRadIn[1];
                b_loc[2] = pRadIn[2] * pRadIn[2];
                if (internal::minimum(b_loc) <= pq.D[pq.D.size(0) - 1]) {
              ...
              if ((pq.D.size(0) != numNN) || (!ballIsWithinBounds)) {
                ...
                while (nodeStack.size(0) != 0) {
                  ...
                  if ((pq.D.size(0) < numNN) || vision::internal::codegen::
                      Kdtree::boundsOverlapBall(b_loc, lowBounds, upBounds,
                       pq.D[pq.D.size(0) - 1], static_cast<double>(X.size(1))))
                  {
                    if (!b_this->LeafNode[static_cast<int>(currentNode) - 1]) {
                      nxout = static_cast<int>(b_this->CutDim[static_cast<int>
                        (currentNode) - 1]) - 1;
                      if (loc[j + loc.size(0) * nxout] <= b_this->CutVal[
                          static_cast<int>(currentNode) - 1]) {
                        c_this.set_size(nodeStack.size(0) + 2);
                        c_this[0] = b_this->LeftChild[static_cast<int>
                          (currentNode) - 1];
                        ...
                        c_this[0] = b_this->RightChild[static_cast<int>
                          (currentNode) - 1];
                        c_this[1] = b_this->LeftChild[static_cast<int>
                          (currentNode) - 1];
                        yk = nodeStack.size(0);
                        for (nxout = 0; nxout < yk; nxout++) {
                          c_this[nxout + 2] = nodeStack[nxout];
              ...
              for (nxout = 0; nxout < yk; nxout++) {
                indices[(r1[nxout] + indices.size(0) * j) - 1] = pq.b_I[nxout];
              }
      ...
      normals.set_size(loc.size(0), 3);
      PCANormalImpl_single(&loc[0], &indices[0], &(valid.data())[0],
                           static_cast<unsigned int>(loc.size(0)), static_cast<
...
//
// File trailer for pointCloud.cpp
//
// [EOF]
//
