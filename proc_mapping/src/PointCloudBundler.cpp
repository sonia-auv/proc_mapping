...
bool PointCloudBundler::step()
{
  ...
  geometry_msgs_PoseWithCovarianceStruct_T t8_Pose;
  double t17;
  double t18;
  double t19;
  double t20;
    ...
    if (out) {
      ...
      if (out) {
        ...
        mSonarSub->get_LatestMessage(&sonarMsg_Height, &sonarMsg_Width,
                                     ...
                                     sonarMsg_Data);
        mPoseSub->get_LatestMessage(&t8_Pose);
        ...
        fflush(stdout);
        // fix
        qnrm = ((t8_Pose.Pose.Orientation.W * t8_Pose.Pose.Orientation.W +
                 t8_Pose.Pose.Orientation.X * t8_Pose.Pose.Orientation.X) +
                t8_Pose.Pose.Orientation.Y * t8_Pose.Pose.Orientation.Y) +
               t8_Pose.Pose.Orientation.Z * t8_Pose.Pose.Orientation.Z;
        quat_idx_0 = t8_Pose.Pose.Orientation.W / qnrm;
        quat_idx_1 = -t8_Pose.Pose.Orientation.X / qnrm;
        quat_idx_2 = -t8_Pose.Pose.Orientation.Y / qnrm;
        quat_idx_3 = -t8_Pose.Pose.Orientation.Z / qnrm;
        ...
        if (nx - 1 >= 0) {
          ...
          t6 = quat_idx_1 * quat_idx_3 * 2.0;
          t7 = quat_idx_2 * quat_idx_3 * 2.0;
          t8 = quat_idx_0 * quat_idx_1 * 2.0;
          t9 = quat_idx_0 * quat_idx_2 * 2.0;
          qnrm = quat_idx_0 * quat_idx_3 * 2.0;
          quat_idx_1 = quat_idx_1 * quat_idx_1 * 2.0;
          quat_idx_2 = quat_idx_2 * quat_idx_2 * 2.0;
          quat_idx_0 = quat_idx_3 * quat_idx_3 * 2.0;
          t17 = t5 + qnrm;
          t18 = t6 + t9;
          t19 = t7 + t8;
          t20 = t5 + -qnrm;
          t21 = t6 + -t9;
          t7 += -t8;
          t8 = (quat_idx_1 + quat_idx_2) - 1.0;
          t9 = (quat_idx_1 + quat_idx_0) - 1.0;
          t6 = (quat_idx_2 + quat_idx_0) - 1.0;
        }
        for (i2 = 0; i2 < nx; i2++) {
          qnrm = xyzi[i2];
          t5 = xyzi[i2 + xyzi.size(0)];
          ...
          xyzi[i2] = (((((t8_Pose.Pose.Position.X - 0.358 * t6) + 0.0 * t20) +
                        -0.118 * t18) -
                       t6 * qnrm) +
                      t20 * t5) +
                     t18 * 0.0;
          xyzi[i2 + xyzi.size(0)] =
              (((((t8_Pose.Pose.Position.Y + 0.358 * t17) - 0.0 * t9) +
                 -0.118 * t7) +
                t17 * qnrm) -
               t9 * t5) +
              t7 * 0.0;
          xyzi[i2 + xyzi.size(0) * 2] =
              (((((t8_Pose.Pose.Position.Z + 0.358 * t21) + 0.0 * t19) -
                 -0.118 * t8) +
                t21 * qnrm) +
               t19 * t5) -
              t8 * 0.0;
