                     ...
                     pointCloud *iobj_1)
{
  ...
  loop_ub = nv.size(0) * nv.size(1);
  for (int i{0}; i < loop_ub; i++) {
    iobj_1->Normal[i] = nv[i];
  }
  iobj_1->Intensity.set_size(b_intensity.size(0));
  loop_ub = b_intensity.size(0);
  for (int i{0}; i < loop_ub; i++) {
    iobj_1->Intensity[i] = b_intensity[i];
  }
  ...
  iobj_1->RangeData.set_size(normal.size(0), normal.size(1));
  loop_ub = normal.size(0) * normal.size(1);
  for (int i{0}; i < loop_ub; i++) {
    iobj_1->RangeData[i] = normal[i];
  }
  return ptCloudOut;
}
...
void c_pointCloud::multiQueryKNNSearchImpl(
        ...
        if (!wasNaNY[j]) {
          ...
          if (pq.D.size(0) != 0) {
            ...
            if (lowBounds.size(1) == 3) {
              ...
              binary_expand_op(pRadIn, points, j, lowBounds);
            }
            if (upBounds.size(1) == 3) {
              ...
              binary_expand_op(b_pRadIn, points, j, upBounds);
            }
