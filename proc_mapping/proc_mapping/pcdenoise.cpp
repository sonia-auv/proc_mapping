                      ...
                      pointCloud *iobj_1)
{
  ...
  iobj_1->Color.set_size(c.size(0), c.size(1));
  npages = c.size(0) * c.size(1);
  for (i = 0; i < npages; i++) {
    iobj_1->Color[i] = c[i];
  }
  iobj_1->Normal.set_size(dists.size(0), dists.size(1));
  npages = dists.size(0) * dists.size(1);
  for (i = 0; i < npages; i++) {
    iobj_1->Normal[i] = dists[i];
  }
  iobj_1->Intensity.set_size(absdiff.size(0));
  npages = absdiff.size(0);
  for (i = 0; i < npages; i++) {
    iobj_1->Intensity[i] = absdiff[i];
  }
  pointclouds::internal::codegen::pc::pointCloudArray r3;
  iobj_1->RangeData.set_size(0, 0);
  iobj_1->PointCloudArrayData.set_size(1, 1);
  iobj_1->PointCloudArrayData[0] = r3;
  iobj_1->Kdtree = iobj_0;
  iobj_1->matlabCodegenIsDeleted = false;
  iobj_1->RangeData.set_size(b_r.size(0), b_r.size(1));
  npages = b_r.size(0) * b_r.size(1);
  for (i = 0; i < npages; i++) {
    iobj_1->RangeData[i] = b_r[i];
  }
  return ptCloudOut;
}
