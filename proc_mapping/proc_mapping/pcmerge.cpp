                    ...
                    pointCloud *iobj_1)
{
  ...
  if ((pointsA.size(0) == 0) && (pointsB.size(0) == 0)) {
    int loop_ub;
    ptCloudOut = &iobj_1[0];
    iobj_1[0].Location.set_size(0, 3);
    iobj_1[0].Color.set_size(colorA.size(0), colorA.size(1));
    loop_ub = colorA.size(0) * colorA.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[0].Color[i] = colorA[i];
    }
    iobj_1[0].Normal.set_size(overlapLimits.size(0), overlapLimits.size(1));
    loop_ub = overlapLimits.size(0) * overlapLimits.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[0].Normal[i] = overlapLimits[i];
    }
    iobj_1[0].Intensity.set_size(intensityA.size(0));
    loop_ub = intensityA.size(0);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[0].Intensity[i] = intensityA[i];
    }
    pointclouds::internal::codegen::pc::pointCloudArray r;
    iobj_1[0].RangeData.set_size(0, 0);
    iobj_1[0].PointCloudArrayData.set_size(1, 1);
    iobj_1[0].PointCloudArrayData[0] = r;
    iobj_1[0].Kdtree = &iobj_0[0];
    iobj_1[0].matlabCodegenIsDeleted = false;
    iobj_1[0].RangeData.set_size(rangeDataA.size(0), rangeDataA.size(1));
    loop_ub = rangeDataA.size(0) * rangeDataA.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[0].RangeData[i] = rangeDataA[i];
    }
  } else if ((pointsA.size(0) == 0) && (pointsB.size(0) != 0)) {
    int loop_ub;
    ptCloudOut = &iobj_1[1];
    iobj_1[1].Location.set_size(pointsB.size(0), 3);
    loop_ub = pointsB.size(0) * 3;
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[1].Location[i] = pointsB[i];
    }
    iobj_1[1].Color.set_size(colorB.size(0), colorB.size(1));
    loop_ub = colorB.size(0) * colorB.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[1].Color[i] = colorB[i];
    }
    iobj_1[1].Normal.set_size(rangeData.size(0), rangeData.size(1));
    loop_ub = rangeData.size(0) * rangeData.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[1].Normal[i] = rangeData[i];
    }
    iobj_1[1].Intensity.set_size(intensityB.size(0));
    loop_ub = intensityB.size(0);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[1].Intensity[i] = intensityB[i];
    }
    pointclouds::internal::codegen::pc::pointCloudArray r;
    iobj_1[1].RangeData.set_size(0, 0);
    iobj_1[1].PointCloudArrayData.set_size(1, 1);
    iobj_1[1].PointCloudArrayData[0] = r;
    iobj_1[1].Kdtree = &iobj_0[1];
    iobj_1[1].matlabCodegenIsDeleted = false;
    iobj_1[1].RangeData.set_size(rangeDataB.size(0), rangeDataB.size(1));
    loop_ub = rangeDataB.size(0) * rangeDataB.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[1].RangeData[i] = rangeDataB[i];
    }
  } else if ((pointsB.size(0) == 0) && (pointsA.size(0) != 0)) {
    int loop_ub;
    ptCloudOut = &iobj_1[2];
    iobj_1[2].Location.set_size(pointsA.size(0), 3);
    loop_ub = pointsA.size(0) * 3;
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[2].Location[i] = pointsA[i];
    }
    iobj_1[2].Color.set_size(colorA.size(0), colorA.size(1));
    loop_ub = colorA.size(0) * colorA.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[2].Color[i] = colorA[i];
    }
    iobj_1[2].Normal.set_size(overlapLimits.size(0), overlapLimits.size(1));
    loop_ub = overlapLimits.size(0) * overlapLimits.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[2].Normal[i] = overlapLimits[i];
    }
    iobj_1[2].Intensity.set_size(intensityA.size(0));
    loop_ub = intensityA.size(0);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[2].Intensity[i] = intensityA[i];
    }
    pointclouds::internal::codegen::pc::pointCloudArray r;
    iobj_1[2].RangeData.set_size(0, 0);
    iobj_1[2].PointCloudArrayData.set_size(1, 1);
    iobj_1[2].PointCloudArrayData[0] = r;
    iobj_1[2].Kdtree = &iobj_0[2];
    iobj_1[2].matlabCodegenIsDeleted = false;
    iobj_1[2].RangeData.set_size(rangeDataA.size(0), rangeDataA.size(1));
    loop_ub = rangeDataA.size(0) * rangeDataA.size(1);
    for (int i{0}; i < loop_ub; i++) {
      iobj_1[2].RangeData[i] = rangeDataA[i];
    }
  } else {
    double d;
    double d1;
    ...
    if ((overlapLimits.size(0) == 0) || (overlapLimits.size(1) == 0)) {
      ptCloudOut = &iobj_1[3];
      iobj_1[3].Location.set_size(points.size(0), 3);
      loop_ub = points.size(0) * 3;
      for (int i{0}; i < loop_ub; i++) {
        iobj_1[3].Location[i] = points[i];
      }
      iobj_1[3].Color.set_size(color.size(0), color.size(1));
      loop_ub = color.size(0) * color.size(1);
      for (int i{0}; i < loop_ub; i++) {
        iobj_1[3].Color[i] = color[i];
      }
      iobj_1[3].Normal.set_size(normal.size(0), normal.size(1));
      loop_ub = normal.size(0) * normal.size(1);
      for (int i{0}; i < loop_ub; i++) {
        iobj_1[3].Normal[i] = normal[i];
      }
      iobj_1[3].Intensity.set_size(intensity.size(0));
      loop_ub = intensity.size(0);
      for (int i{0}; i < loop_ub; i++) {
        iobj_1[3].Intensity[i] = intensity[i];
      }
      pointclouds::internal::codegen::pc::pointCloudArray r;
      iobj_1[3].RangeData.set_size(0, 0);
      iobj_1[3].PointCloudArrayData.set_size(1, 1);
      iobj_1[3].PointCloudArrayData[0] = r;
      iobj_1[3].Kdtree = &iobj_0[3];
      iobj_1[3].matlabCodegenIsDeleted = false;
      iobj_1[3].RangeData.set_size(rangeData.size(0), rangeData.size(1));
      loop_ub = rangeData.size(0) * rangeData.size(1);
      for (int i{0}; i < loop_ub; i++) {
        iobj_1[3].RangeData[i] = rangeData[i];
      }
    } else {
