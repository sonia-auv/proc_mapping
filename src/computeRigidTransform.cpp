                           ...
                           float t[3])
{
  ...
  if (q.size(0) == 0) {
    ...
    if (q.size(0) <= 1024) {
      firstBlockLength = q.size(0);
      lastBlockLength = 0;
      nblocks = 1;
    } else {
      firstBlockLength = 1024;
      nblocks = q.size(0) >> 10;
      lastBlockLength = q.size(0) - (nblocks << 10);
      if (lastBlockLength > 0) {
        nblocks++;
      } else {
        lastBlockLength = 1024;
    ...
    for (int xi{0}; xi < 3; xi++) {
      xpageoffset = xi * q.size(0);
      centroid2[xi] = q[xpageoffset];
      for (int k{2}; k <= firstBlockLength; k++) {
        centroid2[xi] += q[(xpageoffset + k) - 1];
      }
      for (ib = 2; ib <= nblocks; ib++) {
        double b_bsum;
        xblockoffset = xpageoffset + ((ib - 1) << 10);
        b_bsum = q[xblockoffset];
        if (ib == nblocks) {
          hi = lastBlockLength;
        } else {
          hi = 1024;
