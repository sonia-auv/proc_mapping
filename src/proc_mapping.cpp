...
void proc_mapping()
{
  ...
  while (1) {
    ...
    if (ptBundler.lastBundleState && (!bundleStarted)) {
      ...
      for (i = 0; i < 3; i++) {
        for (int nrowx{0}; nrowx < nx; nrowx++) {
      ...
      for (nx = 0; nx < i; nx++) {
        ...
        x[0] = XYZ[nx];
        x[1] = XYZ[nx + XYZ.size(0)];
        x[2] = XYZ[nx + XYZ.size(0) * 2];
      ...
      if (out && bundleStarted) {
        ...
        if ((v.size(0) == b_xyzi.size(0)) && (i == r3.size(0))) {
          nx = v.size(0);
          for (i = 0; i < nx; i++) {
            v[i] = (v[i] + b_xyzi[i]) + r3[i];
          }
        } else {
          b_binary_expand_op(v, b_xyzi, r3);
        }
        nx = v.size(0);
        for (int k{0}; k < nx; k++) {
          v[k] = std::sqrt(v[k]);
        }
        if (xyzi.size(0) == v.size(0)) {
          nx = xyzi.size(0);
          c_xyzi.set_size(xyzi.size(0));
          for (i = 0; i < nx; i++) {
            c_xyzi[i] = ((xyzi[i + xyzi.size(0) * 3] < 0.07) && (v[i] > 5.0));
          }
          coder::any(c_xyzi, idx);
        } else {
          binary_expand_op(idx, xyzi, v);
        }
        nrowx = xyzi.size(0);
        nx = 0;
        i = idx.size(0);
        ...
        for (i = 0; i < 4; i++) {
          for (nrowx = 0; nrowx < nx; nrowx++) {
            xyzi[nrowx + nx * i] = xyzi[nrowx + xyzi.size(0) * i];
          }
        }
        xyzi.set_size(nx, 4);
